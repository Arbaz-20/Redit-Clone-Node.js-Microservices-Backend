...
export class VoteRepository {
  public async currentValue(userId: string, targetType: string, targetId: string): Promise<number> {
  ...
  public async clear(userId: string, targetType: string, targetId: string) {
    await db.delete(votes).where(and(eq(votes.userId, userId), eq(votes.targetType, targetType), eq(votes.targetId, targetId)));
  }
  public async upsert(id: string, userId: string, targetType: string, targetId: string, value: number) {
  ...
  public async score(targetType: string, targetId: string): Promise<number> {
