...
async function hydrate(ids: string[]): Promise<FeedPost[]> {
  const out: FeedPost[] = [];
  for (const id of ids) {
...
export const feedService = {
  async indexPost(p: { id: string; communityId: string; authorUsername: string; title: string; createdAt: string }) {
    ...
    await feedRepository.addToRanked(p.id, hotRank(0, createdAtMs));
    await feedRepository.addToTop(p.id, 0);
    logger.info(`Indexed post ${p.id} into feed`);
  },

  async applyVote(postId: string, delta: number) {
    ...
    await feedRepository.addToRanked(postId, hotRank(newScore, createdAtMs));
  ...
  async getTopFeed(limit: number): Promise<FeedPost[]> {
